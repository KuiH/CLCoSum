...
def get_warmup_steps(initial_rate:float, warmup_rate:float, all_data_one_epoch_steps:int, train_epochs:int, scheduler_fun):
    ...
    for e in range(train_epochs-1):
        next_train_begin_ratio, next_train_end_ratio = scheduler_fun(e+1, train_epochs-1, initial_rate)
        next_train_begin_ratio, next_train_end_ratio = min(1, next_train_begin_ratio), min(1, next_train_end_ratio)
        all_train_ratio += next_train_end_ratio - next_train_begin_ratio
...
class IOTool:
    ...
    def dump_jsonl(obj_list:List[object], out_path:str):
        """将obj_list变为jsonl格式，输出到outpath中"""

...
MODEL_CLASSES = {'roberta': (RobertaConfig, RobertaModel, RobertaTokenizer, Seq2Seq)}
...
def main():
    ...
    config_class, model_class, tokenizer_class, seq2seq_class = MODEL_CLASSES[args.model_type]
    ...
    model=seq2seq_class(encoder=encoder,decoder=decoder,config=config,
    ...
    if args.do_train:
        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                logger.info("  ***** Running bleu evaluation on eval data*****")
                logger.info("  Num examples = %d", len(eval_examples))
                logger.info("  Batch size = %d", args.eval_batch_size)
                eval_sampler = SequentialSampler(eval_data)
            ...
            next_train_ratio = min(1, SCHEDULER_FUN[args.scheduler_fun](epoch+1, args.num_train_epochs-1, initial))
    ...
    if args.do_test:
        ...
        test_data = TextDataset(test_features,args) 
        logger.info("  Num examples = %d", len(test_examples))

        # Calculate bleu
        test_sampler = SequentialSampler(test_data)
        test_dataloader = DataLoader(test_data, sampler=test_sampler, batch_size=args.eval_batch_size, num_workers=4, pin_memory=True)
